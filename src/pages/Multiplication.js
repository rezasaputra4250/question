...
const Multiplication = ({ playerData, setPlayerData }) => {
  ...
  const handleSubmit = () => {
    ...
    if (Number(userAnswer) === correctAnswer) {
      ...
      let newSkillExp = (playerData.skills?.perkalian?.exp || 0) + 10;
      let newSkillLevel = Math.floor(newSkillExp / 100) + 1;

      if (!updatedSkills.perkalian) {
        updatedSkills.perkalian = {
          exp: newSkillExp,
          level: newSkillLevel,
        };
        setSkillMessage('Skill Perkalian berhasil ditambahkan!');
        setShowSkillModal(true);
      } else {
        updatedSkills.perkalian.exp = newSkillExp;
        updatedSkills.perkalian.level = newSkillLevel;
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-lg w-96">
        <h1 className="text-2xl font-semibold text-center mb-4">Ujian Perkalian</h1>
        <SoalPenjumlahan
          num1={num1}
          num2={num2}
        ...
        />
        <Timer timer={timer} />
        <SkillSection skillData={playerData.skills?.perkalian || { level: 0, exp: 0 }} />
        <LevelSection level={playerData.level} exp={playerData.exp} />
        <HPSection hp={playerData.hp} />
      </div>

      {/* Skill Modal */}
      <SkillSection skillData={playerData.skills?.perkalian || { level: 0, exp: 0 }} skillMessage={skillMessage} />
