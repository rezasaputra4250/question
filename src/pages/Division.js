...
const Division = ({ playerData, setPlayerData }) => {
  const [num1, setNum1] = useState(0);
  const [num2, setNum2] = useState(1); // Set num2 to 1 to avoid division by 0
  ...
  const generateRandomProblem = useCallback(() => {
    ...
    let numDigits = level;
    let num1 = Math.floor(Math.random() * Math.pow(10, numDigits)) + 1;  // Ensure num1 is always positive
    let num2 = Math.floor(Math.random() * Math.pow(10, numDigits)) + 1;  // Ensure num2 is never zero
    setNum1(num1);
    setNum2(num2);
    setTimer(60);
    startTimer();
  ...
  const handleSubmit = () => {
    ...
    if (Number(userAnswer) === correctAnswer) {
      ...
      let newSkillExp = (playerData.skills?.pembagian?.exp || 0) + 10;
      let newSkillLevel = Math.floor(newSkillExp / 100) + 1;

      if (!updatedSkills.pembagian) {
        updatedSkills.pembagian = {
          exp: newSkillExp,
          level: newSkillLevel,
        };
        setSkillMessage('Skill Pembagian berhasil ditambahkan!');
        setShowSkillModal(true);
      } else {
        updatedSkills.pembagian.exp = newSkillExp;
        updatedSkills.pembagian.level = newSkillLevel;
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-lg w-96">
        <h1 className="text-2xl font-semibold text-center mb-4">Ujian Pembagian</h1>
        <SoalPenjumlahan
          num1={num1}
          num2={num2}
        ...
        />
        <Timer timer={timer} />
        <SkillSection skillData={playerData.skills?.pembagian || { level: 0, exp: 0 }} />
        <LevelSection level={playerData.level} exp={playerData.exp} />
        <HPSection hp={playerData.hp} />
      </div>

      {/* Skill Modal */}
      <SkillSection skillData={playerData.skills?.pembagian || { level: 0, exp: 0 }} skillMessage={skillMessage} />
